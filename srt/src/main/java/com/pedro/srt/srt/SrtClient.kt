        ...
        if (!isStreaming || isRetry) {
            ...
            job = scope.launch {
                ...
                val error = runCatching {
                    ...
                    // 1. Determine which flags to send based on your setting
                    var extensionFlags = ExtensionContentFlag.TSBPDSND.value or
                            ExtensionContentFlag.TSBPDRCV.value or
                            ExtensionContentFlag.TLPKTDROP.value or
                            ExtensionContentFlag.CRYPT.value

                    if (stats.isRetransmissionEnabled.get()) {
                        // Only add these flags if we actually want the server to manage retransmissions
                        extensionFlags = extensionFlags or
                                ExtensionContentFlag.PERIODICNAK.value or
                                ExtensionContentFlag.REXMITFLG.value
                    }

                    // 2. Pass the dynamic flags to the handshake
                    commandsManager.writeHandshake(socket, response.copy(
                        ...
                        handshakeExtension = HandshakeExtension(
                            flags = extensionFlags, // Use the dynamic flags here
                ...
                when (srtPacket) {
                    ...
                    is Nak -> {
                        if(stats.isRetransmissionEnabled.get()) {
                            ...
                            // Always log the NAKs received for debugging
                            stats.nakPacketsReceived.addAndGet(packetsLost.size)
